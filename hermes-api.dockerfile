...
#COPY ./src/Hermes.Host.Api/Hermes.Host.Api.csproj ./Hermes.Host.Api/
...
# publish projects
RUN dotnet publish ./src/Hermes.Host.Api/Hermes.Host.Api.csproj --configuration Release --no-build --output /target/publish
...
# executable to run
ENTRYPOINT [ "dotnet", "Hermes.Host.Api.dll" ]